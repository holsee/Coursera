...
object FunSets {
  ...
  def forall(s: Set, p: Int => Boolean): Boolean = {
    def iter(a: Int): Boolean = {
      if (a > bound) true // terminate after last element  
      else if (s(a) && !p(a)) false // exit early on first failure
      else iter(a+1) // iterate with increment
    }
    iter(-bound)
  }
   ...
   */
  def map(s: Set, f: Int => Int): Set = e => exists(s, y => f(y) == e)
