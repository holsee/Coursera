...
class Replicator(val replica: ActorRef) extends Actor {
  ...
  def receive: Receive = {
    case replicate @ Replicate(key, valueOption, id) => {
      val seq = nextSeq; acks += seq -> (sender, replicate)
      val repeater = context.system.scheduler.schedule(
        0 millis, 250 millis, replica, Snapshot(key, valueOption, seq))
      repeaters += seq -> repeater
    }
    case SnapshotAck(key, seq) => {
      if (acks.contains(seq)) {
        val (sender, Replicate(key, _, id)) = acks(seq)
        acks -= seq; repeaters(seq).cancel(); repeaters -= seq
